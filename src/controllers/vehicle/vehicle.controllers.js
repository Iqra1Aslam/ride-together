  ...
  try {
    ...
    is_nearestVehicle: asyncHandler(async (req, res) => {
      try {
        ...
        console.log('Passenger Location:', passengerLocation); // Debug statement
        console.log('Requested Date:', requestedDate); // Debug statement
        console.log('Time Range:', timeBefore, timeAfter); // Debug statement
    
        const nearbyRides = await PublishRide.aggregate([
          {
            $geoNear: {
              near: { type: "Point", coordinates: passengerLocation }, // Ensure the location format
          ...
          },
          {
            $match: {
              ...
              numSeats: { $gte: 1 } // Ensure at least one seat is available
            }
          },
          {
            $project: {
        ...
        res.status(500).json({ error: 'Failed to find nearby rides', details: error.message });
      }
    }),
...
//     // Update status to 'booked' for found rides
//     if (nearbyRides.length > 0) {
